...
const safeUser = u => u ? ({id:u.id,name:u.name,email:u.email,role:u.role,organizationId:u.organizationId}) : null;
function auth(req,res,next){ const token=(req.headers.authorization||'').replace('Bearer ',''); const uid=sessions.get(token); const user=readStore().users.find(u=>u.id===uid); if(!user) return res.status(401).json({error:'Sign in required.'}); req.user=user; next(); }
function owner(req,res,next){ if(req.user?.role!=='owner') return res.status(403).json({error:'Owner access required.'}); next(); }
function canManage(user,event){ return user.role==='owner' || event.organizationId===user.organizationId; }
...
app.post('/api/create-checkout-session', async (req,res)=>{
  try{
    ...
    const org=d.organizations.find(o=>o.id===e.organizationId); const feePct=Number(process.env.PLATFORM_FEE_PERCENT||d.settings.platformFeePercent||5); if(org?.stripeAccountId){ sessionConfig.payment_intent_data={application_fee_amount:Math.round((subtotal-discountAmount)*feePct/100),transfer_data:{destination:org.stripeAccountId}}; }
    const session=await stripe.checkout.sessions.create(sessionConfig);
    d.orders.push({id:orderId,eventId:e.id,organizationId:e.organizationId,stripeSessionId:session.id,buyerName:req.body.customer?.name||'',buyerEmail:req.body.customer?.email||'',items:normalized,amountSubtotal:subtotal,discountAmount,amountTotal:subtotal-discountAmount,status:'pending',tickets:[],createdAt:new Date().toISOString()}); writeStore(d); res.json({url:session.url});
...
app.listen(port,()=>console.log(`KVN Live Tickets v2 running at ${baseUrl}`));
