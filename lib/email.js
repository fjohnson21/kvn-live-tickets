...
export async function sendDiscipleWelcome({disciple,link,apiKey,from='Kingdom Vibe Network <info@kvnlive.com>',idempotencyKey='',fetchImpl=fetch,now=()=>new Date().toISOString()}) {
  ...
  try{
    ...
    const response=await fetchImpl('https://api.resend.com/emails',{method:'POST',headers,body:JSON.stringify({from,to:[disciple.email],...content})});
    const data=await response.json().catch(()=>({}));
...
export async function sendDiscipleApplicationNotice({application,status,reason,applicationUrl='https://kvnlive.com/disciples',apiKey,from='Kingdom Vibe Network <info@kvnlive.com>',idempotencyKey='',fetchImpl=fetch,now=()=>new Date().toISOString()}) {
  if(!apiKey)return {status:'not_configured',provider:'resend',failedAt:now(),error:'RESEND_API_KEY is not configured.'};
  if(!application?.email)return {status:'failed',provider:'resend',failedAt:now(),error:'Applicant email is required.'};
  const needsInfo=status==='needs_info',name=application.preferredName||application.name||application.legalFirstName||'Applicant';
  const subject=needsInfo?'Action needed for your Kingdom Disciple application':'Update on your Kingdom Disciple application';
  const intro=needsInfo?'We need additional information before we can continue reviewing your application.':'Thank you for your interest in becoming a Kingdom Disciple. We are not able to approve your application at this time.';
  const text=`Hello ${name},\n\n${intro}\n\nAdministrator note:\n${reason}\n\n${needsInfo?`Complete the current application and Agreement v2.2 here: ${applicationUrl}\n\n`:''}Kingdom Vibe Network`;
  const html=`<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#111"><p style="color:#e2252b;font-weight:700;letter-spacing:2px">KINGDOM VIBE DISCIPLE</p><h1>${escapeHtml(needsInfo?'Action needed':'Application update')}</h1><p>Hello ${escapeHtml(name)},</p><p>${escapeHtml(intro)}</p><div style="padding:16px;background:#f4f4f4;border-radius:8px"><strong>Administrator note</strong><p>${escapeHtml(reason)}</p></div>${needsInfo?`<p><a href="${escapeHtml(applicationUrl)}">Complete the current application and Agreement v2.2</a></p>`:''}<p>Kingdom Vibe Network</p></div>`;
  const headers={Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'};if(idempotencyKey)headers['Idempotency-Key']=idempotencyKey;
  try{const response=await fetchImpl('https://api.resend.com/emails',{method:'POST',headers,body:JSON.stringify({from,to:[application.email],subject,text,html})});const data=await response.json().catch(()=>({}));if(!response.ok)return {status:'failed',provider:'resend',failedAt:now(),error:`Resend rejected application notice (${response.status}): ${data.message||data.error||'Unknown provider error'}`};return {status:'sent',provider:'resend',messageId:data.id||'',sentAt:now()};}
  catch(error){return {status:'failed',provider:'resend',failedAt:now(),error:`Application notice delivery failed: ${error.message}`};}
}
