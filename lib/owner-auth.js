...
export function createOwnerPasswordStore({ dataDir, fallbackPassword = '' }) {
  ...
  return {
    isConfigured() { return Boolean(read() || fallbackPassword); },
    verify(password) {
      const stored = read();
      if (stored) return verifyRecord(password, stored);
      const left = crypto.createHash('sha256').update(String(password)).digest('hex');
      const right = crypto.createHash('sha256').update(String(fallbackPassword)).digest('hex');
      return Boolean(fallbackPassword) && safeEqual(left, right);
    },
    set(password) {
