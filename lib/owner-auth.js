...
export function createOwnerPasswordStore({ dataDir, bootstrapPassword = '' }) {
  ...
  if (!read() && bootstrapPassword) {
    const temporary = `${file}.${process.pid}.tmp`;
    fs.writeFileSync(temporary, JSON.stringify(passwordRecord(bootstrapPassword)), { mode: 0o600 });
