...
export function refundShopOrder(store,orderId,{clock=()=>new Date()}={}){
  ...
  const commission=store.discipleCommissions?.find(item=>item.orderId===order.id&&item.sourceType==='apparel');
  if(commission){commission.status='reversal_required';commission.reversalNote='Apparel order refunded. Reverse or offset this reward.';}
  return order;
}
